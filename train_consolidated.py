...
#
#   REVISIONS:
#   - Added stability improvements to RefinedLayer (clamping, epsilons).
#   - Implemented a gated loss mechanism to modulate the listnet loss
...
class RefinedLayer(nn.Module):
    ...
    def forward(self, edge_index, h):
        H = self.embed(h) if self.embed else h
        s = h[:, :self.struct_dim]
        s_feat = s[:, :-1]
        ...
        # --- MODIFIED: Numerical stability improvements ---
        # 1. Clamp score before exponentiation to prevent overflow
        score = (h_tgt * self.W_att(h_src)).sum(-1) + (s_src * s_tgt).sum(-1)
        score = torch.clamp(score, -10, 10) # Clamp raw scores
        ...
        alpha = alpha_num / (alpha_den[tgt] + 1e-9)
        ...
        if not self.disable_pairing:
            ...
            pair = torch.clamp(pair, -10, 10) # Clamp again
            ...
            sum_term = scatter_sum(beta * torch.exp(-torch.clamp(psi, -10, 10)), rev_tgt,
                                   dim=0, dim_size=H.size(0))
            # 3. Add epsilon to log argument
...
class RefinedAFGCN(nn.Module):
    def __init__(self, struct_dim=7, hidden=128, layers=2, **abl_flags):
        ...
        self.ranking_head = nn.Linear(hidden, 1)
...
def load_pyg_baselines():
    ...
    return dict(
        ...
        gin       = lambda h,l: mod.GIN(in_dim=128, hid=h, out_dim=1),
...
# --- MODIFIED: Pass mcc_gate to control rank loss ---
def train_epoch(model, loader, loss_fn, opt, scaler, device,
                refined=False, use_rank=False, lam_rank=0.1, mcc_gate=0.0):
    model.train()
    ...
    for data in pbar:
        ...
        with autocast(device_type="cuda" if device.type == "cuda" else "cpu"):
            if refined:
                class_logits, rank_scores = model(data)
                loss = loss_fn(class_logits, data.y.float())
                if use_rank:
                    # Apply the MCC-based gate to the ranking loss
                    rank_loss = listnet_loss(rank_scores, data.rank.to(device))
                    loss += lam_rank * mcc_gate * rank_loss
            else: # baselines
                x_in = pad_inputs(data.x)
                class_logits = model(Data(x=x_in, edge_index=data.edge_index))
                loss = loss_fn(class_logits, data.y.float())
        ...
        del loss, class_logits
        if refined: del rank_scores
...
def evaluate(model, loader, loss_fn, device, refined=False, desc="val"):
    ...
    with torch.no_grad(), autocast(device_type=device.type):
        for data in pbar:
            ...
            loss_sum += loss_fn(class_logits, data.y.float()).item()
            pred = (torch.sigmoid(class_logits) > 0.5).float()
...
def main():
    ...
    p.add_argument('--rank_loss_weight', type=float, default=0.1,
                   help='λ in BCE + λ·g·ListNet; ignored if --no_rank_loss')
    ...
    opt, scaler = torch.optim.Adam(model.parameters(), lr=args.lr), GradScaler()
    best_mcc = -1.0
    mcc_gate = 0.0 # Initialize gate to 0

    for epoch in range(1, args.epochs + 1):
        t0 = time.time()
        # --- MODIFIED: Pass the gate value to the training loop ---
        ...
        # --- MODIFIED: Update the gate based on validation MCC ---
        # The gate is the validation MCC from the *previous* epoch.
        # It's scaled from [-1, 1] to [0, 1] to act as a multiplier.
        # An MCC of 0 or less results in a gate of 0, disabling rank loss.
        mcc_gate = max(0.0, mcc)
        
        print(f"Epoch {epoch:03d} ┃ acc {acc:.4f}  mcc {mcc:.4f}  "
              f"val_loss {vloss:.4f}  rank_gate {mcc_gate:.2f}  [{dt/60:.1f} min]")
